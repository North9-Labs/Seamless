...
pub fn self_signed_acceptor(domains: &[&str]) -> Result<TlsAcceptor> {
    ...
    let cert = params
        ...
        .context("signing self-signed cert")?;
...
pub fn admin_tls_acceptor(
    ...
    let cert_pem =
        std::fs::read(cert_path).with_context(|| format!("reading admin TLS cert {cert_path}"))?;
    let key_pem =
        std::fs::read(key_path).with_context(|| format!("reading admin TLS key {key_path}"))?;

    let server_certs: Vec<CertificateDer<'static>> =
        certs(&mut BufReader::new(cert_pem.as_slice()))
            .collect::<Result<Vec<_>, _>>()
    ...
    let config = if let Some(ca_path) = client_ca_path {
        ...
        let ca_pem =
            std::fs::read(ca_path).with_context(|| format!("reading admin client CA {ca_path}"))?;
